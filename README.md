...
“各大学学生数量统计”按入学高校聚合 `enrollments`（学生人数按学生去重、
录取人次含重复记录），展示 Top N / 全部横向柱状图与按高校性质汇总，
支持性质筛选；数据同样由同源 JSON 端点从 web_db 预聚合读取。
...
- 位于一级菜单"系统设置"下的"数据库初始化"页面
- 点击"启动初始化"后弹出警告确认框——该操作会**删除并重建** web_db / log_db
  两个数据库及 web_user / log_user 两个账号，全部现有数据不可恢复
- 确认后经由 Celery 平台派发 `tasks.db.init_web_db` 任务（alt_celery3），
  页面实时显示进度，任务完成后展示重建的数据库、账号、业务表清单等信息
- 本应用不再包含任何建表（DDL）代码，数据库结构统一由平台任务初始化；
  应用启动时也不会自动建表，需先执行一次"数据库初始化"
